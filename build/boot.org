...
* build.boot
  ...
  Файл ~build.boot~ исполняется Boot при запуске src_shell{boot task}, где ~task~ имя задачи для исполнения. Задачи проекта
...
*** clj-build
    ...
    Для вызова задачи выполните: src_shell[:dir ".."]{boot clj-build}
...
*** clj-run
    ...
    Для вызова задачи выполните: src_shell[:dir ".."]{boot clj-run}
...
*** cljs-figwheel
    ...
    Эта задача интерфейс к *figwheel*, для запуска используйте src_shell{boot cljs-figwheel}.\\
    ...
    1. выполнить src_shell{boot cljs-figwheel}
