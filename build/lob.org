...
* in-src-path
  ...
  *Примеры:*
  #+BEGIN_EXAMPLE
    #+BEGIN_SRC clojure :tangle (org-sbe in-src-path (param="Расположение clj-исходников" path \"core.clj\"))
       ...
    #+END_SRC
    <<in-src-path(param="Расоложение clj-исходников", path="core.clj")>>
    #+CALL: in-src-path(param="Расположение clj-исходников", path="core.clj")
  #+END_EXAMPLE
...
* in-clj-path
  ...
  *Примеры:*
  ...
  | Имя  | Значение по умолчанию | Описание                                    |
  |------+-----------------------+---------------------------------------------|
  | path | ""                    | Путь отнистельно дирректории clj-исходников |
  #+NAME: in-clj-path
  #+BEGIN_SRC emacs-lisp :var path="" :results value silent
    (save-excursion
      (org-babel-execute-src-block
        nil
        (cdr (assoc 'in-src-path org-babel-library-of-babel))
        (list
...
* in-cljs-path
  ...
  *Примеры:*
  ...
  | Имя  | Значение по умолчанию | Описание                                     |
  |------+-----------------------+----------------------------------------------|
  | path | ""                    | Путь отнистельно дирректории cljs-исходников |
  #+NAME: in-cljs-path
  #+BEGIN_SRC emacs-lisp :var path="" :results value silent
    (save-excursion
      (org-babel-execute-src-block
